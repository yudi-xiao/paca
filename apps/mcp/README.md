...
3. Only tools represented by both the enrolled capability list and a requested Grant scope are exposed. Current tools cover exact project reads, task reads, single-field task writes, constrained task creation, executable-task discovery, document snapshots, and version-checked text block edits.
...
6. `discover_tasks` reports Host/Harness presence before discovery so server-side matching can use approved labels and current online state.
