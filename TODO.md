...
- [ ] 完成长任务恢复、重复投递、取消、Harness 失联和权限中途撤销测试。当前本地运行时已覆盖 Workflow step 重试、Grant 拒绝、取消竞态、Host 离线匹配和 AgentDO 驱逐恢复；远程 Workflow smoke 覆盖成功、完全重复投递、变更请求冲突和 Grant 撤销，远程 Task Harness smoke 已覆盖 claim/checkpoint 幂等、重启续接、lease 到期重领、人工取消幂等、取消后续租拒绝、变更 request ID 冲突、竞争领取、checkpoint 跳号、完成和 Grant 撤销。剩余项仅为通过真实 scheduled Cron 触发 Host heartbeat-expiry 的远端验收。
