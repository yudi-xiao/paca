...
`paca/internal` 已应用 `0024_deep_nomad.sql` 与 `0025_moaning_wild_pack.sql`，新增 Project-scoped 环境映射并允许 `cloudflare-sandbox` provider。Environment Gateway 使用稳定版 `@cloudflare/sandbox@0.12.9`、固定容器镜像 digest、独立 Secret、一次性 execute 票据和最长 55 秒连接权限，部署版本为 `8e4e51e4-8579-4125-9a3f-78bb64494d43`；API Worker 版本为 `d7e8f55f-922c-4771-83ab-0820dddbf8d0`。
...
- [ ] 使用可控的远端 provider transient/capacity 故障注入，或捕获真实发生的可重试平台故障，验证线上调用方确实等待、重试并仍保持同一业务 request ID；当前远端冷启动一次成功，不能作为“实际发生重试”的证据。
