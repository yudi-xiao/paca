...
`paca/internal` 已应用 `0024_deep_nomad.sql` 与 `0025_moaning_wild_pack.sql`，新增 Project-scoped 环境映射并允许 `cloudflare-sandbox` provider。Environment Gateway 使用稳定版 `@cloudflare/sandbox@0.12.9`、固定容器镜像 digest、独立 Secret、一次性 execute 票据和最长 55 秒连接权限，部署版本为 `01a408ea-fdcf-4bb7-b910-4a7af3156a82`；API Worker 版本为 `944cbab2-24be-48a2-ae3d-825112b70e34`。
...
真实 local Harness smoke 已覆盖 read 与 execute：Agent Auth 注册/审批、精确 Grant、Agent JWT、Service Binding 签票、`Sandbox.listProcesses`、PTY 双向帧、同票据重放拒绝、Grant 撤销后的新连接拒绝、Agent/Project 栅栏对撤销前票据的拒绝，以及 Grant 撤销或 delegated 用户失去 Project 连接权限后的已建立 PTY 服务端关闭；Project 权限 smoke 同时确认 Grant 保持 active，从而没有把权限变化误实现为 Grant 撤销。测试完成后均清理 Agent、用户 Session、数据库测试数据及临时角色。Worker 67 个文件/360 项、Workers Runtime 4 个文件/22 项、Gateway 14 项测试，以及类型、Biome、Drizzle、dry-run、真实部署和双 health 均通过；瞬时 provider 失败细分、调用方重试预算和实际环境管理 UI 尚待完成。
...
- [ ] 完成容器冷启动/provider 平台错误分类和调用方受控重试远端 E2E；当前 API 已将作用域/授权错误映射为 403、Gateway 暂时不可用/响应错误映射为 503。
