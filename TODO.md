...
当前里程碑：**M9 的 Environment 资源管理与版本化 Gateway 已形成可用纵向切片。**

`paca/internal` 已应用 `0024_deep_nomad.sql`、`0025_moaning_wild_pack.sql` 与 `0026_nosy_gamma_corps.sql`，新增 Project-scoped 环境映射、`cloudflare-sandbox` provider，以及名称/创建者/活动名称唯一约束。Environment Gateway 使用稳定版 `@cloudflare/sandbox@0.12.9`、固定容器镜像 digest、独立 Secret、一次性 execute 票据和最长 55 秒连接权限，部署版本为 `fd2a5e13-e6b4-4bb9-aa0a-94c718f818f5`；API Worker 与 React Web 版本为 `510fb8c0-7b20-487e-9108-72108e3096a8`。

每个 execute 连接使用独立 Sandbox Session，并在按 Agent ID 稳定命名的 registry DO 中只登记 provider 引用、Session ID 和最长 15 分钟的授权失效时间。撤销 `environment.connect` Grant 会立即经私有 Service Binding 删除该 Agent 的全部活动 Session；Project 角色/成员变更会在 Paca Permission 同一权威入口比较变更前后有效权限，只终止真正失去 `environments.read` 或 `environments.connect` 的 delegated Agent 在该 Project 的 Session。项目归档会终止所有持有该 Project 精确 active `environment.connect` Grant 的 Agent Session。Agent/Project 主体栅栏与按 Environment UUID 稳定命名的全局票据栅栏共同拒绝撤销前已经签发但尚未使用的票据；全局 DO 只保存时间戳，即使没有历史 Agent Grant，环境归档也会推进该栅栏，为浏览器用户连接提供相同的 fail-closed 基础。provider 删除失败仍由 DO alarm 以 5～60 秒指数退避继续回收。

真实 local Harness smoke 已覆盖 read 与 execute：Agent Auth 注册/审批、精确 Grant、Agent JWT、Service Binding 签票、`Sandbox.listProcesses`、PTY 双向帧、同票据重放拒绝、Grant 撤销后的新连接拒绝、Agent/Project/Environment 全局栅栏对撤销前票据的拒绝，以及 Grant 撤销或 delegated 用户失去 Project 连接权限后的已建立 PTY 服务端关闭；Project 权限 smoke 同时确认 Grant 保持 active，从而没有把权限变化误实现为 Grant 撤销。Gateway 已加入稳定、脱敏的 provider 错误信封和显式 `prepare` 预热，Worker API 与本地 Harness 只在服务端明确标记可重试时使用同一业务 request ID 做最多 3 次重试，不会重放结果不确定的终端写操作。唯一 Sandbox 引用的真实冷启动 execute smoke 已通过，`prepare` 用时 2792ms、Gateway 与调用方均为 1 次尝试，并再次验证 PTY、重放保护、Grant 撤销和连接。随后以临时 Service Binding Worker 并发竞争正式 `max_instances=5`：首轮 5 个成功、2 个暴露 SDK 跨 DO 边界只保留 `ContainerUnavailableError` 类名的真实兼容问题；修复后同一业务 request ID 的两次调用均稳定返回 `503 GATEWAY_PROVIDER_TRANSIENT`，调用方确实等待并重试，且没有把 SDK 已消耗的约 120 秒预算在 Gateway 内再次放大。环境资源 CRUD 和 internal UI 已部署：用户只控制名称，provider/reference 由服务端生成，公开状态为 `ready_on_demand`；归档写入环境全局票据栅栏及精确主体撤销栅栏并可幂等重试。最新真实 Environment archive smoke 使用全新 Sandbox，`prepare` 用时 2773ms，确认已有 PTY 关闭、预签票据拒绝、active Grant 不变且归档后新连接拒绝。真实 Session smoke 已验证列表、受限创建、重复名 409、详情、重命名、归档、归档后隐藏和归档重试，测试环境已清理。Worker 69 个文件/381 项、Workers Runtime 4 个文件/22 项、Gateway 4 个文件/29 项、Web 61 个文件/634 项测试，以及类型、Biome、Drizzle、绑定类型、dry-run 和真实部署均通过。
...
- [ ] 将 `services/agent-runner` 从 legacy `PACA_API_KEY` 身份迁移到本机 Agent Host 配置，完成 delegated Agent 注册、device approval、短期 `host+jwt`/Agent JWT 轮换和 Project-scoped Capability 执行；完成前不得把 Host enrollment 宣称为 Runner 已接通。Go Runner 已加入未部署的 Agent Auth 协议客户端、在线状态和任务执行 lease 切片：严格读取 `0600` delegated Agent 配置并验证同源端点/Ed25519 密钥，每次请求生成唯一 `jti` 的 45 秒 Agent JWT，启动时同步验证 `task.execute` 心跳并按 10～60 秒周期上报 `cloudflare-agent`、Codex、Claude Code、DeepSeek 或 custom Harness；匹配该身份的 `task_assigned` trigger 必须从服务端 discovery 取得精确 Organization/Project/Task scope，随后 claim/resume、半周期 renew、sandbox-ready checkpoint，并在结束时 complete/fail，lease 控制失效会取消执行。其他 Agent 在迁移窗口继续走旧路径，不能被单一身份越权领取。`apps/mcp` 已加入未部署的双认证入口：受控本机 Harness 可直接读取 `0600` Agent 配置，以每请求新鲜 Agent JWT 调用按 Grant 精确收敛的 project/task/document/workflow/environment 工具，`discover_tasks` 会先上报 Harness presence；工具暴露同时要求 capability 与对应 Grant request，Project/Task/Document/Environment/Workflow/field/operationMode/action/有效期均在本地 fail-closed 并由服务端再次权威校验。文档工具读取 revision、Yjs state vector 和 block version，只允许 `suggest`/`collaborate` 下最多 10 个文本 block 的细粒度版本检查修改，拒绝整篇覆盖；Workflow 工具必须同时持有匹配的 `workflow.execute` 与 `document.edit` Grant，可启动、查询和取消 Cloudflare durable run；Environment 工具只返回服务端签发的短时连接结果并要求 Harness 不记录或持久化 access token。MCP 共 561 项测试通过。Agent 私钥不会下发到 Computer/Sandbox；Runner sandbox 仍需实现短期 Capability Broker 并删除 legacy key，因此本项继续保持未勾选。
...
- [x] 为浏览器 Environment 连接补齐主体无关的撤销基础：新增按不可变 Environment UUID 稳定命名的 `EnvironmentTicketBarrierDO`，只保存最后撤销时间戳；Gateway 使用票据时并行检查该全局栅栏与现有 Agent registry 栅栏。环境归档即使没有历史 Agent Grant 也必须通过私有 Service Binding 推进全局栅栏，通知失败以 503 fail closed 并可幂等重试。Gateway migration `v3-environment-ticket-barrier`、空主体协议测试和真实 Agent PTY 归档 smoke 均通过；浏览器用户主体、Session 连接 API 和终端 UI 仍属下一切片。
