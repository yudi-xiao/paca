...
`paca/internal` 已应用 `0024_deep_nomad.sql` 与 `0025_moaning_wild_pack.sql`，新增 Project-scoped 环境映射并允许 `cloudflare-sandbox` provider。Environment Gateway 使用稳定版 `@cloudflare/sandbox@0.12.9`、固定容器镜像 digest、独立 Secret、一次性 execute 票据和最长 55 秒连接权限，部署版本为 `97821555-973e-453a-8f93-ab2b1629974b`；API Worker 版本为 `8438fe21-3e89-4a3b-8de6-6c5b29e36edf`。

每个 execute 连接使用独立 Sandbox Session，并在按 Agent ID 稳定命名的 registry DO 中只登记 provider 引用、Session ID 和最长 15 分钟的授权失效时间。撤销 `environment.connect` Grant 会立即经私有 Service Binding 删除该 Agent 的活动 Session；失败时由 DO alarm 以 5～60 秒指数退避继续回收。

真实 local Harness smoke 已覆盖 read 与 execute：Agent Auth 注册/审批、精确 Grant、Agent JWT、Service Binding 签票、`Sandbox.listProcesses`、PTY 双向帧、同票据重放拒绝、Grant 撤销后的新连接拒绝、撤销前已签发票据的 Gateway 栅栏拒绝和已建立 PTY 服务端关闭，并清理 Agent、用户 Session、数据库测试数据及临时角色。Worker 66 个文件/354 项、Workers Runtime 4 个文件/22 项、Gateway 13 项测试，以及类型、Biome、Drizzle、dry-run、真实部署和双 health 均通过；瞬时 provider 失败细分、调用方重试预算和实际环境管理 UI 尚待完成。
...
- [ ] 完成 Project 权限撤销后的已建立环境连接终止，以及容器冷启动/provider 平台错误分类和调用方受控重试远端 E2E；当前 API 已将作用域/授权错误映射为 403、Gateway 暂时不可用/响应错误映射为 503。
