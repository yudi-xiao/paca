...
当前里程碑：**M9 的 Environment 资源管理、Agent/用户双主体 Gateway 与浏览器终端已形成可用纵向切片。**

`paca/internal` 已应用 `0024_deep_nomad.sql`、`0025_moaning_wild_pack.sql` 与 `0026_nosy_gamma_corps.sql`，新增 Project-scoped 环境映射、`cloudflare-sandbox` provider，以及名称/创建者/活动名称唯一约束。Environment Gateway 使用稳定版 `@cloudflare/sandbox@0.12.9`、固定容器镜像 digest、独立 Secret、一次性 execute 票据和最长 55 秒连接权限，部署版本为 `b3e159ce-34e4-44f7-a416-a55c93f33388`；API Worker 与 React Web 版本为 `8efa9dc6-8f2f-45ae-ab40-7bc082d23c77`。

每个 execute 连接使用独立 Sandbox Session，并在按 Agent ID 或 Better Auth User ID 稳定命名、namespace 分离的 registry DO 中只登记 provider 引用、Session ID 和最长 15 分钟的授权失效时间。撤销 `environment.connect` Grant 会立即经私有 Service Binding 删除该 Agent 的全部活动 Session；Better Auth sign-out 会在注销 Session 前删除该用户的全部活动 Sandbox Session。Project 角色/成员变更会在 Paca Permission 同一权威入口比较变更前后有效权限，同时终止真正失去 `environments.read` 或 `environments.connect` 的用户和 delegated Agent 在该 Project 的 Session。项目归档会覆盖项目成员与所有持有该 Project 精确 active `environment.connect` Grant 的 Agent。主体栅栏与按 Environment UUID 稳定命名的全局票据栅栏共同拒绝撤销前已经签发但尚未使用的票据；provider 删除失败仍由 DO alarm 以 5～60 秒指数退避继续回收。

真实 local Harness smoke 已覆盖 read 与 execute：Agent Auth 注册/审批、精确 Grant、Agent JWT、Service Binding 签票、`Sandbox.listProcesses`、PTY 双向帧、同票据重放拒绝、Grant 撤销后的新连接拒绝、Agent/Project/Environment 全局栅栏对撤销前票据的拒绝，以及 Grant 撤销或 delegated 用户失去 Project 连接权限后的已建立 PTY 服务端关闭；Project 权限 smoke 同时确认 Grant 保持 active，从而没有把权限变化误实现为 Grant 撤销。Gateway 已加入稳定、脱敏的 provider 错误信封和显式 `prepare` 预热，Worker API 与本地 Harness 只在服务端明确标记可重试时使用同一业务 request ID 做最多 3 次重试，不会重放结果不确定的终端写操作。唯一 Sandbox 引用的真实冷启动 execute smoke 已通过，`prepare` 用时 2792ms、Gateway 与调用方均为 1 次尝试，并再次验证 PTY、重放保护、Grant 撤销和连接。随后以临时 Service Binding Worker 并发竞争正式 `max_instances=5`：首轮 5 个成功、2 个暴露 SDK 跨 DO 边界只保留 `ContainerUnavailableError` 类名的真实兼容问题；修复后同一业务 request ID 的两次调用均稳定返回 `503 GATEWAY_PROVIDER_TRANSIENT`，调用方确实等待并重试，且没有把 SDK 已消耗的约 120 秒预算在 Gateway 内再次放大。环境资源 CRUD 和 internal UI 已部署：用户只控制名称，provider/reference 由服务端生成，公开状态为 `ready_on_demand`；归档写入环境全局票据栅栏及精确主体撤销栅栏并可幂等重试。最新真实 Edge 验收以 Better Auth Session 创建 `Demo Sandbox`，项目卡片打开用户主体网页终端，按需冷启动后返回 `PACA_BROWSER_TERMINAL_OK`；退出登录后连接变为“已断开连接”，后续命令没有执行。Worker 69 个文件/384 项、Workers Runtime 4 个文件/22 项、Gateway 4 个文件/33 项、Web 61 个文件/637 项测试，以及类型、Biome、Drizzle、绑定类型、dry-run 和真实部署均通过。
...
- [x] internal 已退出原 Hyperdrive 的宽权限 role：新建无继承管理角色的 `paca-worker-internal`，现仅显式授予 47 张 runtime 业务表 CRUD，验证其能读取业务表且不能读取 migration ledger；独立 Hyperdrive 已创建并接收 internal 流量。根/main Hyperdrive 仅保留为独立环境与 Wrangler 版本回滚路径。
- [x] 已固化 runtime role 的显式 47 表 CRUD GRANT 与验权 SQL；目标最小权限 role 无 DDL 和 migration ledger/附件迁移账本权限，PlanetScale 授权时使用去掉路由后缀的真实 role 名。现有宽权限 role 的验权会按预期失败，不能作为生产验收结果。
...
- [x] 已实现并实际执行受确认串保护的 `database:provision:internal`：检查 main/internal migration ledger、拒绝分叉目标、以单事务和 `ON CONFLICT DO NOTHING` 初始复制、核对应用表行数与业务表指纹、应用/验证最小权限 role 并创建或更新独立 Hyperdrive；当前权限验收清单为 47 张 runtime 表。脚本不输出密码，临时 admin role 15 分钟自动过期；2026-08-31 已完成获批的数据复制与 runtime role 凭据轮换，Task 自引用外键在提交前恢复为 `NOT DEFERRABLE`。
...
- [x] 0023 Workspace Branding migration 已在 `paca/internal` 通过同一受控流程应用：新增 singleton 设置与不可变 R2 上传状态两表、引用完整性和清理索引；runtime role 已扩为 46 表 CRUD 且仍无 DDL/ledger 权限。真实 Session/R2 远端 smoke 已验证设置、上传、激活、公开读取、删除与恢复；`paca/main` 仍停在 0014。
- [x] 0024–0026 Environment migrations 已在 `paca/internal` 通过同一受控流程应用：新增 `paca_environment_scope`，精确绑定环境 UUID、Project、provider 类型和 opaque Gateway reference，随后前滚支持 `cloudflare-sandbox` 并补充名称、创建者、项目内活动名称唯一索引与名称约束。三项 ledger checksum、列/索引、迁移权限和临时 role reassign/delete 均已核验；runtime role 仍为 47 表 CRUD，`paca/main` 仍停在 0014。
...
- [x] 建立 migration dry-run/测试数据库流程：Worker CI 使用隔离的本地 `paca_worker_test` PostgreSQL，从空 `public` schema 顺序应用全部受版本控制 migration 并核对完整 ledger；脚本强制 localhost 与 `_test` 库名，不读取根 `.env`，生产启动仍不执行 `push` 或自动迁移。GitHub Actions `34433080779` 已用 PostgreSQL 17 从空 schema 实际应用 27 个 migration 并通过后续 dry-run bundle。
- [x] 建立 PostgreSQL repository contract test 基础设施：首个 `ProjectRepository` 共用契约覆盖聚合 CRUD、分页/统计、大小写唯一性与子记录失败时的事务回滚；PostgreSQL adapter 在上述全新迁移数据库中执行，后续 D1 adapter 可复用同一领域契约。GitHub Actions `34433080779` 的 3 项真实 PostgreSQL contract tests 全部通过。
...
- [x] 实现 autonomous Agent 模式：Host enrollment 仅在配置至少 32 字符 bootstrap Secret 时开放；Host/Agent 权威记录保持 `userId = null`，Better Auth 仅为请求期 Session 解析合成且不落库的 subject；Project approver 授予最小 Capability 后，执行始终以可信 Agent ID 作为业务与持久审计 actor，若存在 delegated user 只作为委托上下文保留。真实 Agent Auth 0.6.2 协议测试覆盖错误 Secret、注册、审批、active Grant、精确 constraints、执行、撤销和撤销后拒绝。
...
- [ ] 为 document/environment/workflow 接入对应领域执行器。Document read/edit 与 document Workflow 已完成 Worker 执行边界：Better Auth Agent Auth 先验证 JWT/active Grant/constraints，再校验真实 Document scope；Workflow run 使用独立 `workflow.execute` 与 `document.edit` Grant、持久编排及按 Agent ID 稳定命名的 AgentDO，本地 Harness MCP 可启动、查询和取消 run。Environment 已完成 PostgreSQL 资源 CRUD、internal 管理 UI、Agent Auth executor、私有 Service Binding、稳定 Cloudflare Sandbox provider，以及 Grant/Project/Environment 三层连接撤销。尚需通用业务 Workflow 领域执行器、浏览器终端/文件/SSH/Port Forward 契约和旧环境迁移，因此总项不勾选。
...
- [x] Autonomous Agent 只依赖审批后仍 active 的 Grant 和 constraints；领域执行器仍校验真实 Organization/Project/Task/Document/Environment 归属，既不查询也不伪造普通用户权限，过期 Grant、错误 scope 和撤销后的新 JWT 均 fail closed。
...
- [ ] 将 `services/agent-runner` 从 legacy `PACA_API_KEY` 身份迁移到本机 Agent Host 配置，完成 delegated Agent 注册、device approval、短期 `host+jwt`/Agent JWT 轮换和 Project-scoped Capability 执行；完成前不得把 Host enrollment 宣称为 Runner 已接通。Go Runner 已加入未部署的 Agent Auth 协议客户端、在线状态和任务执行 lease 切片：严格读取 `0600` delegated Agent 配置并验证同源端点/Ed25519 密钥，每次请求生成唯一 `jti` 的 45 秒 Agent JWT，启动时同步验证 `task.execute` 心跳并按 10～60 秒周期上报 `cloudflare-agent`、Codex、Claude Code、DeepSeek 或 custom Harness；匹配该身份的 `task_assigned` trigger 必须从服务端 discovery 取得精确 Organization/Project/Task scope，随后 claim/resume、半周期 renew、sandbox-ready checkpoint，并在结束时 complete/fail，lease 控制失效会取消执行。其他 Agent 在迁移窗口继续走旧路径，不能被单一身份越权领取。`apps/mcp` 已加入三种互斥认证入口：受控本机 Harness 可直接读取 `0600` Agent 配置，以每请求新鲜 Agent JWT 调用按 Grant 精确收敛的 project/task/document/workflow/environment 工具；Runner 管理的临时 Sandbox 只接收 256-bit opaque bearer 和不含私钥/JWT/Host 端点的 Project-scoped 公开 Grant 摘要，由进程内 Capability Broker 校验固定 Project 路由、Capability、方法、请求大小及 body 后代签新鲜 Agent JWT，Broker 以 SHA-256 索引 session，bearer 不持久化或记录日志、45 分钟空闲失效并在 teardown 立即撤销；legacy key 只供 rollout identity 之外的 Agent。静态 Environment 已通过 Goose Secret Store 完成每 conversation/每 turn 刷新：所有 Paca MCP 值写入 conversation 唯一键名，`session/new`/`session/load` 仅引用键名，MCP 激活后立即清理，turn 结束立即撤销 bearer，并显式清除子进程继承的 legacy auth。Broker 拒绝路径规范化逃逸和任意 header/URL 转发，且不下放由 Runner lease 掌握的 `task.execute` 或会返回嵌套 bearer 的 `environment.connect`。文档工具读取 revision、Yjs state vector 和 block version，只允许 `suggest`/`collaborate` 下最多 10 个文本 block 的细粒度版本检查修改，拒绝整篇覆盖；Workflow 工具必须同时持有匹配的 `workflow.execute` 与 `document.edit` Grant，可启动、查询和取消 Cloudflare durable run。MCP 共 569 项测试通过。尚未完成 Runner/身份配置部署、managed Sandbox 的 repository/plugin Agent Auth capability，以及全局删除 legacy key 路径，因此本项继续保持未勾选。
...
- [x] Autonomous Agent 的 Grant 可审计、可撤销且最小化；授予/撤销必须由具有 `agents.approveGrant` 的真实用户完成，持久审计区分审批用户与执行 Agent，撤销同步写入审计并触发 Project/Document/Environment 失效边界，真实协议测试验证撤销后立即拒绝新执行。
...
当前 M9 API/Web 版本为 internal Worker `8efa9dc6-8f2f-45ae-ab40-7bc082d23c77`，Environment Gateway 版本为 `b3e159ce-34e4-44f7-a416-a55c93f33388`。固定 Document Agent Workflow `00000000-0000-4000-8000-000000000201` 已由受 Better Auth Agent Auth 保护的 Hono API 创建、查询和取消；Workflow 参数不包含 JWT、私钥、文档正文或完整 Grant，只保存精确 Grant ID、受限 scope 和结构化命令。`AgentCoordinator` 使用精确锁定的 `agents@0.22.0`，Agents SDK state 只包含有界 run 摘要；Cloudflare 托管 task lease mirror 也只保存受限作用域与状态。`task.execute` lease 核心允许 Cloudflare Agent 和本地 Codex、Claude Code、DeepSeek Harness 通过同一 Agent Auth 执行边界领取指定任务；PostgreSQL 是 lease/checkpoint/event 权威。需要隔离执行时使用独立 Environment Gateway，其首个生产候选 provider 是稳定版 Cloudflare Sandbox；Computer 继续保留为达到生产门槛后的可替换 provider。
...
- [x] 评估并选择执行环境。Cloudflare 托管 Agent 采用 Agents SDK AgentDO + Workflows + Agent Tracing，本地 Codex、Claude Code、DeepSeek harness 使用各自本机执行环境和同一 Agent Auth/任务协议；Harness 类型只参与能力标签与调度，不改变业务授权。2026-09-09 已部署并真实验收稳定版 `@cloudflare/sandbox@0.12.9`/Containers provider，供确需隔离终端的环境连接使用。`@cloudflare/computer` 仍为 preview only / not suitable for production，继续作为 Gateway 后的未来 provider；达到生产门槛后只替换 provider，不改 Agent Auth、Grant 或 Harness 协议。
- [x] 部署版本化私有 Environment Gateway：API Worker 只能通过 Service Binding 的固定内部 origin 签发连接，公开端点必须验证最长 60 秒 HMAC 票据；read 只允许脱敏进程状态，execute 只允许一次 WebSocket PTY，票据、Cookie 和 subprotocol 在进入 Sandbox 前移除。真实 local Harness smoke 已完成 read 与 execute 全链路，等待 PTY `ready` 后以二进制帧发送输入并收到回显，同票据第二次连接被拒绝，测试数据与授权随后清理。
- [x] 完成 `environment.connect` Grant 撤销的执行环境 E2E：撤销后新的 Agent Auth Capability 调用被拒绝，撤销前已签发但未使用的票据也被 registry DO 的持久时间栅栏拒绝；保持 PTY 打开时，API 经私有 Service Binding 通知按 Agent ID 命名的 registry DO 删除独立 Sandbox Session，真实远端连接由服务端关闭。登记状态不保存 JWT、票据、命令或输出，最长随 15 分钟授权失效，由 alarm 回收；删除失败采用 5～60 秒指数退避。
- [x] 完成 Project 权限撤销后的已建立环境连接终止：角色更新、成员换角或移除均从统一 Paca Permission authority 批量读取变更前后有效权限，仅对真正失去 `environments.read`/`environments.connect` 的用户查找 delegated Agent；Project 归档则按精确 active Grant 覆盖 delegated 与 autonomous Agent。API 经私有 Service Binding 批量通知按 Agent ID 命名的 registry DO，Project 级持久栅栏只关闭该 Project 的 Session 并拒绝撤销前票据，不影响同一 Agent 在其他 Project 的连接。真实 execute smoke 通过“移除角色的 `environments.connect`”验证预签票据拒绝、已建立 PTY 服务端关闭、新连接权限交集拒绝且 Grant 仍 active。
- [x] 完成容器冷启动/provider 平台错误分类和调用方受控重试实现：Gateway 将 starting、capacity、transient、operation uncertain、provider failed 与 unsupported 归一为稳定脱敏信封，显式 `prepare` 在消费一次性 WebSocket 票据前完成 Sandbox 就绪探测；API 将授权/作用域拒绝保持为 403，将显式可重试 provider 错误映射为带上限 `Retry-After` 的 503。本地 Harness 只对 `environment.connect` 使用同一 request ID、重新签发短期 Agent JWT 并最多尝试 3 次；read/prepare 可以安全重试，结果不确定的终端或 shell 操作禁止自动重放。唯一 Sandbox 引用的真实冷启动 smoke 已通过，`prepare` 2792ms、两层 attempts 均为 1。
- [x] 完成 Worker 原生 Environment 资源生命周期和 internal 管理 UI：Hono + PostgreSQL 提供 Project-scoped 列表、创建、详情、重命名和软归档，用户输入只允许名称，UUID/provider/opaque Gateway reference 由服务端控制且不出现在响应。Sandbox 状态明确为 `ready_on_demand`；归档会按所有历史精确 Grant 找出可能持票 Agent，在 registry DO 写入 Environment 栅栏并关闭匹配连接，Gateway 失败返回 503 且同一归档可幂等重试。真实 Session CRUD 与 Agent Auth + PTY 环境归档 smoke 均通过；后者确认既有连接被关闭、预签票据被拒绝、active Grant 的新连接也因环境已归档而拒绝。临时环境已物理清理，临时 Agent/Grant 按审计语义保留为 revoked。
- [x] 完成浏览器 Environment 连接与双主体撤销：Hono 在 Better Auth Session 与 `environments.connect` 检查后签发显式 user 票据，先用服务端 warmup 票据经 Service Binding `prepare`，再返回一张全新的浏览器一次性票据；Edge 用 `paca-ticket.<token>` WebSocket 子协议连接，Gateway 在 `101` 回显后剥离凭据再转发 Sandbox 原生 PTY 帧。Agent 与用户使用独立 registry DO namespace，项目权限损失、项目/环境归档和 sign-out 都会关闭匹配连接并使旧票据失效；混合主体撤销按总数 100 分批。真实 Edge 已验证创建 `Demo Sandbox`、按需启动、PTY 命令输出和 sign-out 后主动断开。
- [x] 使用可控远端容量故障完成线上重试 E2E：临时 Worker 通过 Service Binding 并发申请 7 个唯一 Sandbox，正式配置 `max_instances=5` 下 5 个成功、2 个触发真实容量错误。该错误跨 DO/RPC 边界只保留 `ContainerUnavailableError` 类名而丢失 `code/context` getter，已增加精确官方类名 fallback；最终同一 request ID `05bbbe21-32d6-44f7-b502-9daf374a783c` 连续两次均返回 `503 GATEWAY_PROVIDER_TRANSIENT`，`uniqueRequestIds=1`、`callerRetryObserved=true`。两次失败符合容量持续占满预期；临时 Worker、远端预览和 tail 均已删除/停止。
...
- [x] 补齐 Harness 调度面：运行时无关 `AgentTaskHarnessClient`、stdin JSON 本地 CLI、Host 心跳、受审批能力标签、任务 required labels、安全交集匹配和 Cloudflare AgentDO adapter 均已完成。Cloudflare 托管与本地 Harness 共用精确 active Grant/实时 delegated 权限交集和 `task.execute` 入口；自报标签不能扩大 Grant。
...
- [x] 完成长任务恢复、重复投递、取消、Harness 失联和权限中途撤销测试。本地运行时覆盖 Workflow step 重试、Grant 拒绝、取消竞态、Host 离线匹配和 AgentDO 驱逐恢复；远程 Workflow smoke 覆盖成功、完全重复投递、变更请求冲突和 Grant 撤销；远程 Task Harness smoke 覆盖 claim/checkpoint 幂等、重启续接、lease 到期重领、人工取消幂等、取消后续租拒绝、变更 request ID 冲突、竞争领取、checkpoint 跳号、完成和 Grant 撤销。独立真实 recovery smoke 在 lease 尚未自然到期时使 Host heartbeat 失效，scheduled Cron 将其标记 expired 并写 `expire/system` 审计，随后恢复原 Host runtime 和清理测试 lease/role。
...
本切片已完成：Workspace Branding 已由公开空对象替换为 PostgreSQL + R2 的完整设置与图片生命周期；`0023` 为 additive migration。
...
- [x] 将 Workspace Branding 迁移到 Worker：公开设置与不可变图片读取、`settings.write` 管理边界、PostgreSQL singleton/上传状态、R2 流式写入、大小/ETag/magic-byte 完成校验，以及废弃对象/超时上传的定时清理均已实现；真实 Session/R2 全链路和设置恢复已通过。
...
- [x] 为 Worker/Go API 混合期定义明确路由、错误格式和回滚边界：已知容器保留域在 Worker 未实现的路径返回不可缓存的 `501 API_DOMAIN_NOT_MIGRATED` 与稳定 domain header，未知路径仍为 404；Better Auth 与旧 Go principal bridge 未验收前禁止隐式代理，当前只允许按已验证 Worker 版本回滚，因此不存在代理超时的模糊配置。internal 版本 `81948b93-a023-4948-a031-6b507f3f922e` 已远端验证 health 200、未知 API 404、Demo Environment 路径 501 及 `no-store`/domain header。
