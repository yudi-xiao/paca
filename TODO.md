...
当前里程碑：**M9 的版本化 Environment Gateway 已形成可部署纵向切片。`paca/internal` 已应用 `0024_deep_nomad.sql` 与 `0025_moaning_wild_pack.sql`，新增 Project-scoped 环境映射并允许 `cloudflare-sandbox` provider；`paca-environment-gateway-internal` 以稳定版 `@cloudflare/sandbox@0.12.9`、固定容器镜像 digest、独立 Secret、一次性 execute 票据和最长 60 秒连接权限部署为版本 `df928ef1-77a9-4710-962f-c3ecbd917d57`，公开入口为 `paca-env.howlearnwood.com`。API Worker `e71beae8-2717-45a5-9c55-4cf685a38d9f` 已通过私有 Service Binding 接入 Gateway，并把环境领域错误保留为安全的 403/503 Agent Auth 协议码。真实 local Harness smoke 分别完成 read 与 execute 两条链路：Better Auth Agent Auth 注册、设备审批、精确 `environment.connect` Grant、Agent JWT、Service Binding 签票、`Sandbox.listProcesses`、PTY `ready`/二进制输入/回显及同票据重放拒绝，随后清理 Grant、Agent、Session、数据库测试数据和临时角色。Worker 66 个文件/351 项、Workers Runtime 4 个文件/22 项、Gateway 8 项测试，以及类型、Biome、Drizzle、dry-run、真实部署和双 health 均通过；瞬时 provider 失败细分/受控重试、连接中途撤销和实际环境管理 UI 尚待完成。**
...
- [ ] 完成 Grant/Project 权限撤销后的新连接拒绝、已建立连接中途终止和容器冷启动失败分类/受控重试远端 E2E；当前首次 smoke 曾在冷启动返回通用 `internal_error`，API 现已将作用域/授权错误映射为 403、Gateway 暂时不可用/响应错误映射为 503，仍需细分 provider 平台错误并制定重试预算。
