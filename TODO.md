...
- [ ] 补齐 Harness 调度面：运行时无关 `AgentTaskHarnessClient` 和 stdin JSON 本地 CLI adapter 已完成，仍需 Host 心跳与受审批能力标签、可领取任务发现/匹配及 Cloudflare 托管 adapter；不得让 Harness 自报标签扩大 Grant，也不得建立绕过 `task.execute` 的第二套入口。
